...
3. Once assigned, fork the repo and create a branch named:
   ...
   where `N` is the issue number, e.g. `feat/8-add-goal-edit-flow`.
4. Write your code. Make sure `npm run build` and `next lint` both pass locally.
...
- No stub components, TODO comments, or placeholder logic — every PR should ship
  a complete, working implementation of what its issue describes.
- Mobile responsive at 375px+. Loading states for anything that fetches on-chain
  data. Dark theme throughout.
